...
int main() {
    ...
    // Create ramps
    auto ground1 = std::make_shared<ChBodyEasyBox>(10, 0.1, 10, 1000, sph_mat, collision_type);
    ground1->SetPos(ChVector<>(12.299, 1.5, 0));
    ground1->SetBodyFixed(true);
    ground1->GetVisualShape(0)->SetMaterial(0, ramp_mat_vis);
    sys.Add(ground1);

    auto ground2 = std::make_shared<ChBodyEasyBox>(3, 0.1, 10, 1000, sph_mat, collision_type);
    ground2->SetPos(ChVector<>(6, 0.75, 0));
    ground2->SetRot(Q_from_AngZ(CH_C_PI / 6));
    ground2->SetBodyFixed(true);
    ground2->GetVisualShape(0)->SetMaterial(0, ramp_mat_vis);
    sys.Add(ground2);

    auto ground3 = std::make_shared<ChBodyEasyBox>(3, 0.1, 10, 1000, sph_mat, collision_type);
    ground3->SetPos(ChVector<>(18.598, 0.75, 0));
    ground3->SetRot(Q_from_AngZ(-CH_C_PI / 6));
    ground3->SetBodyFixed(true);
    ground3->GetVisualShape(0)->SetMaterial(0, ramp_mat_vis);
    sys.Add(ground3);
